# GatiChowk — Pre-Build Research Synthesis & Build Blueprint
...
| **Counting method** | **Instantaneous per-lane density** (`PolygonZone` / `RegionCounter`) | LineZone crossing counts (secondary) | A stopped queue produces *zero* line-crossings; cumulative counting fails once congestion occupies the zone — GatiChowk's normal condition. |
