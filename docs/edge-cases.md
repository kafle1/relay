...
86. 🟠 **Label smear in the overlay matcher** (free-look reprojection matched each detector box to the nearest sim vehicle, greedy in box order and class-blind — in a queue a "car" box got painted around the bike beside its own car) → globally nearest-first assignment with a half-box class-mismatch penalty: labels prefer their own kind, a mislabeled box still lands on the vehicle under it, and the match runs once per detector result, not per frame.
87. 🟠 **Cross-class suppression before filtering** (agnostic NMS inside predict() ran before the PCU/confidence gates, so a person box — discarded later anyway — or a below-gate detection could eat the real motorcycle/truck box and erase or relabel it) → per-class NMS in predict(), then `cross_class_dedupe` AFTER the gates: only boxes that count can suppress.
88. 🔴 **Auto-label camera drift poisons training** (labels project through the frozen cctvCam but capture saved whatever camera was on screen; the poses matched by accident until they didn't — one run trained a detector that scored its own garbage labels well and saw nothing live; `project()` also mirrored behind-camera cars into fake edge boxes) → capture double-renders through cctvCam exactly like the live stream, and labelFor rejects any box with a corner behind the camera or fully off-frame instead of clamping it into existence.
89. 🟡 **Rare classes rounded down to "car"** (demo spawn weights are Kathmandu-realistic — trucks/buses 6% each, ambulances button-only — so the fine-tune barely saw heavies and defaulted ambiguous vehicles to the majority class) → capture mode flattens the mix (heavies ~35%, organic ambulances 6%) while the demo keeps the realistic weights; retrain gate: verify label alignment visually (`tools/verify_labels.py`) BEFORE training, mAP@50 0.955 after.
90. 🔴 **Pedestrian walks into a vehicle already on the zebra** (exit-side turners legally sweep the crossing during the walker's red; walk windows also opened the instant red began, while the last legal car was still mid-zebra — and walkers had no concept of steel) → three layers: a 1.6 s CLEARANCE before the first walk window; admission refused while a vehicle sits on the first metres of the path; mid-zebra walkers yield to vehicles at 1.1 m — deliberately MORE daylight than the ~0.8 m at which vehicles yield to walkers, so exactly one side ever waits (margin ordering makes mutual freeze impossible).
91. 🟠 **Full-speed instant stops read as crashes** (cross traffic, wedged turners and zebra walkers were not in the braking envelope — a car ran 14 m/s into a hard stop 0.35 m short of the obstacle) → near-stationary bodies + walkers join the v²=2ad envelope via a one-step path probe (braking only when the gap is closing, so a queued adjacent lane never throttles a free lane); the stuck-car knot tiers keep their bounded creep.
92. 🟠 **Detection blinks when YOLO misses a frame** (only boxes matched THIS frame were emitted; a track with miss ≤ 4 was alive but invisible — vehicles' boxes flickered and counts dipped) → unmatched live tracks coast: last box re-emitted, zone-assigned and counted, confidence decayed per missed frame; the client staleness window breathes with measured inference latency.
93. 🔴 **A/B compared luck, not control** (both panels rolled independent `Math.random` — different vehicles at different times made every visible comparison contestable) → per-spawn-event seeded RNG keyed (seed, stream, index); compare.html hands the SAME `?seed` to both panels, so arrivals are identical vehicle-for-vehicle; an early-out in one panel cannot desync the other, and Reset deals a fresh shared hand.
94. 🟠 **A pedestrian stream pins a trickle road green** (the on-zebra hold ran to `max_green` while re-windowed admissions kept `crossing > 0` — the loaded corridor drowned; a live A/B inversion) → `walk_hold` caps the hold at `min_green + 12 s` (one full crossing); stragglers stay safe by construction because vehicles hard-yield to walkers regardless of signals.
95. 🟡 **Crowd-sized walk demand steering the junction** (26 pedestrians' aggregated pressure + per-lane holds outweighed vehicle demand) → population trimmed to 14/10; ped demand stays an input, never the driver.
96. 🔴 **Movers were invisible to movers** (the braking envelope only saw near-stationary bodies, so a green-launcher and a still-clearing straggler — or a follower and the leader that had just entered its turn arc — converged at full speed and slam-froze nose-to-nose at 35 cm, reading as a crash; the stuck-nudge then ground queued bumpers to 8 cm; and a car frozen by the envelope never accrued "stuck", so the escape ladder couldn't reach a true standoff) → four layers, all root-level: the envelope path-probes MY OWN route (arc-aware) against EVERY body and walker at ANY speed — same-heading movers are followed, everything else is a braking target, and path-projection keeps adjacent queues and oncoming pass-bys out of it; the stop line releases nobody while a cross-group vehicle is still in the box (a dynamic all-red no fixed clearance interval can size right); `stuck` is outcome-based — wanted to move, didn't — so every freeze escalates inch → wedge → nudge → tow, and the nudge skips cars merely queued behind their own leader; anyone on a zebra holds that whole approach until the crossing is clear.
