...
% This TeX install has no Courier (pcr). IEEEtran asks for it for \texttt and URLs, so fall
% back to Computer Modern typewriter, which ships with every distribution. IEEE's own
% compile farm has Courier, so this only affects local builds.
...
\title{A Low-Cost Single-Camera Adaptive Signal Controller for Heterogeneous
Urban Traffic: Self-Calibrating Pressure Control with Bounded Fairness and
Emergency Preemption}

\author{Niraj~Kafle%
\thanks{N. Kafle is an independent researcher in Kathmandu, Nepal
(e-mail: contact.me.kafle@gmail.com).}%
\thanks{Source code, experiment scripts and raw result files are available at
...
\begin{IEEEkeywords}
Adaptive traffic signal control, max-pressure control, computer
...
Webster's method \cite{Webster1958TrafficSignalSettings} showed that cycle length and splits should
...
\cite{shrestha2014development,nepali2024assessment}, which is exactly the condition under which a
plan imported from a design manual performs badly.

The established remedy is a demand-responsive system such as SCOOT
\cite{Hunt1981SCOOT,Robertson1991SCOOTMethod} or SCATS \cite{Sims1980SCATS}, or an
optimisation-based controller in the OPAC \cite{Gartner1983OPAC} and RHODES
\cite{Mirchandani2001RHODES} lineage. These work and have decades of field evidence behind them, but
...
\cite{redmon2016yolo,bochkovskiy2020yolov4}, which makes per-approach vehicle counting a software
...
\cite{umair2021efficient,harilakshmi2023deep,abewickrema2025ensemble}, and several recent studies
connect a detector to a signal timer on this basis
\cite{papanashi2024adaptive,alruyshid2025aibased,talaat2025smart,medvei2025deepsignal}.
...
\cite{Varaiya2013MaxPressure,Varaiya2013MaxPressureChapter} provides throughput-optimality results
...
Webster's analysis of cycle length and green splits \cite{Webster1958TrafficSignalSettings} remains
...
\cite{Tassiulas1992Stability}. Varaiya
\cite{Varaiya2013MaxPressure,Varaiya2013MaxPressureChapter} showed that repeatedly activating the
...
work established distributed formulations \cite{Wongpiromsarn2012Distributed} and decentralised
variants with weaker information requirements \cite{Le2015Decentralized}.
...
capacity; Levin et al. \cite{Levin2020CyclicalMaxPressure} address this with a cyclical phase structure,
...
\cite{wei2021recent}, Haydari and Yilmaz \cite{haydari2022deep} and Noaeen et al.
\cite{noaeen2022reinforcement} cover the field; representative works include IntelliLight
\cite{wei2018intellilight}, large-scale decentralised training \cite{chen2020thousand} and
PressLight \cite{wei2019presslight}, whose reward is derived from max-pressure and which is
therefore a direct point of comparison. Early work by Genders and Razavi
\cite{genders2016using} established the basic formulation.
...
\cite{redmon2016yolo,bochkovskiy2020yolov4}, reviewed by Terven et al.
\cite{terven2023comprehensive}, typically trained on general corpora \cite{lin2014microsoft} and
extended with tracking when trajectories are required \cite{bewley2016simple,wojke2017simple}. Our
system uses the Ultralytics implementation of the YOLO family \cite{jocher2023yolov8}; the architecture
generation we fine-tune is described in \cite{khanam2024yolov11}, which remains a preprint. Because the
...
\cite{papanashi2024adaptive} report YOLO-based adaptive timing evaluated in Bengaluru, a
heterogeneous-traffic context comparable to ours; Alruyshid et al. \cite{alruyshid2025aibased} add
emergency and public-bus prioritisation in a Baghdad case study; Talaat et al. \cite{talaat2025smart}
and Medvei et al. \cite{medvei2025deepsignal} report detector-driven controllers, the latter combining
...
\cite{louisell2004simple}; Humagain et al. \cite{humagain2020systematic} review route optimisation and
preemption methods, and recent work exploits connected-vehicle messaging to grant priority with explicit
dilemma-zone protection \cite{das2022traffic}. Our approach differs in its detection channel, since the
...
\cite{chandra2003effect,arasan2010microsimulation,kumar2018new}, but they were not estimated from
...
Webster \cite{Webster1958TrafficSignalSettings} minimises delay at cycle length
...
accumulates at a single point rather than along a link \cite{daganzo1997fundamentals}. While an approach
displays green it discharges at $s = 0.55$~veh/s, a saturation headway of about 1.8~s, within the range
capacity guidance gives for a through lane \cite{trb2022hcm}, accumulated as a fractional carry so a green
...
\cite{lopez2018microscopic} because the controller under test is the system's own code and the purpose is
...
takes cycle length and splits from Webster's formulae \cite{Webster1958TrafficSignalSettings} computed
...
Figure~S1 in the supplementary material shows the same comparison unaggregated, one point per seed.
...
\cite{Levin2020CyclicalMaxPressure} propose a cyclical structure to recover the lost capacity.
...
Figure~S2 in the supplementary material shows what the resulting policy does. \relay{} switches more often than the
...
Table~S1 in the supplementary material reports mean average precision of 0.954 at IoU 0.5 and 0.809 averaged over
...
Figure~S4 in the supplementary material shows annotated frames from both clips, and the second shows the main
perception weakness plainly. To quantify rather than assert it, we measured how the count changes with
inference resolution (Figure~S3 in the supplementary material, right). On the dense clip the mean sampled vehicle
...
\cite{lopez2018microscopic} would settle it.
...
\cite{wei2019presslight} in particular since its reward derives from max-pressure; our reasons for not
